...
class ECKadNode {
public:
    ECKadNode(const ECKadNode&) = delete;
    ECKadNode& operator=(const ECKadNode&) = delete;
    ECKadNode() {
        all_clash_size_ = 0;
        all_used_size_ = 0;
    ...
    void AddNode(uint64_t id) {
        uint64_t nid = AppendNodeID(id);
        node_maps_->insert(std::pair<uint64_t, Node*>(nid, new Node(nid)));
    }
    ...
    void PushData(uint64_t objid, uint64_t _size) {
        ...
        uint64_t objid1 = Rand60() | (*header_iter << 60);
        header_iter++;
        uint64_t objid2 = Rand60() | (*header_iter << 60);
        header_iter++;
        uint64_t objid3 = Rand60() | (*header_iter << 60);
        header_iter++;
        uint64_t objid4 = Rand60() | (*header_iter << 60);
        header_iter++;
        uint64_t objid5 = Rand60() | (*header_iter << 60);
        header_iter++;
        uint64_t objid6 = Rand60() | (*header_iter << 60);
        header_iter++;
        uint64_t objid7 = Rand60() | (*header_iter << 60);
        header_iter++;
        uint64_t objid8 = Rand60() | (*header_iter << 60);
